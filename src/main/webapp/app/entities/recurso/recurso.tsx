...
import { getEntities } from './recurso.reducer';
import { IRecurso } from 'app/shared/model/recurso.model';
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';
import { overridePaginationStateWithQueryParams } from 'app/shared/util/entity-utils';

export interface IRecursoProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}
        ...
        {recursoList && recursoList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  <Translate contentKey="cidhaApp.recurso.tipoRecurso">Tipo Recurso</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="cidhaApp.recurso.opcaoRecurso">Opcao Recurso</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="cidhaApp.recurso.processo">Processo</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {recursoList.map((recurso, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {recurso.tipoRecurso ? <Link to={`tipo-recurso/${recurso.tipoRecurso.id}`}>{recurso.tipoRecurso.descricao}</Link> : ''}
                  </td>
                  <td>
                    {recurso.opcaoRecurso ? (
                      <Link to={`opcao-recurso/${recurso.opcaoRecurso.id}`}>{recurso.opcaoRecurso.descricao}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="cidhaApp.recurso.home.notFound">No Recursos found</Translate>
            </div>
          )
        )}
      </div>
      {props.totalItems ? (
        <div className={recursoList && recursoList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ recurso }: IRootState) => ({
  recursoList: recurso.entities,
  loading: recurso.loading,
  totalItems: recurso.totalItems,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(Recurso);
