...
import { getEntities } from './fundamentacao-legal.reducer';
import { IFundamentacaoLegal } from 'app/shared/model/fundamentacao-legal.model';
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';
import { overridePaginationStateWithQueryParams } from 'app/shared/util/entity-utils';

export interface IFundamentacaoLegalProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}
        ...
        {fundamentacaoLegalList && fundamentacaoLegalList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('folhasFundamentacaoLegal')}>
                  <Translate contentKey="cidhaApp.fundamentacaoLegal.folhasFundamentacaoLegal">Folhas Fundamentacao Legal</Translate>{' '}
                  <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('fundamentacaoLegalSugerida')}>
            ...
            <tbody>
              {fundamentacaoLegalList.map((fundamentacaoLegal, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{fundamentacaoLegal.fundamentacaoLegal}</td>
                  <td>{fundamentacaoLegal.folhasFundamentacaoLegal}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="cidhaApp.fundamentacaoLegal.home.notFound">No Fundamentacao Legals found</Translate>
            </div>
          )
        )}
      </div>
      {props.totalItems ? (
        <div className={fundamentacaoLegalList && fundamentacaoLegalList.length > 0 ? '' : 'd-none'}>
...
const mapStateToProps = ({ fundamentacaoLegal }: IRootState) => ({
  fundamentacaoLegalList: fundamentacaoLegal.entities,
  loading: fundamentacaoLegal.loading,
  totalItems: fundamentacaoLegal.totalItems,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(FundamentacaoLegal);
